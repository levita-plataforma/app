/**
 * Estilos y utilidades compartidas del módulo Grupos. Se replica el enfoque de
 * Eventos (src/app/(app)/app/eventos/ui.ts): estilos en línea sobre las clases
 * ya existentes de la shell (`shell-card`, `serving-table`, `serving-chip`…),
 * sin hoja de estilos propia, para no introducir una identidad visual distinta
...
/** Valor por defecto de un `<input type="datetime-local">` ("YYYY-MM-DDTHH:MM"). */
...
/** Aforo legible. Los responsables no ocupan plaza (decisión P-2). */
export function formatOccupancy(memberCount: number, capacity: number | null): string {
  return capacity === null ? String(memberCount) : `${memberCount}/${capacity}`;
}
