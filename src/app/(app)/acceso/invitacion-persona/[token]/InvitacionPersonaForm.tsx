...
import { aceptarInvitacionPersonaAction, type AceptarInvitacionPersonaState } from "./actions";
import { authInputStyle, authLabelStyle, authPrimaryButtonStyle } from "@/components/shell/AuthCard";

const initialState: AceptarInvitacionPersonaState = { error: null };

export default function InvitacionPersonaForm({ token }: { token: string }) {
  const boundAction = aceptarInvitacionPersonaAction.bind(null, token);
  ...
  return (
    <form action={formAction} style={{ display: "flex", flexDirection: "column", gap: 14 }} noValidate>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        <input id="password" name="password" type="password" autoComplete="new-password" minLength={8} required style={authInputStyle} />
      </div>
